...
if (substr(tails_get_theme_option('blog_style'), 0, 7) == 'masonry') {
...
if (have_posts()) {
	...
	$tails_classes = 'posts_container '
						. (substr(tails_get_theme_option('blog_style'), 0, 7) == 'classic' ? 'columns_wrap' : 'masonry_wrap');
	...
	if (!$tails_sticky_out) {
		if (tails_get_theme_option('first_post_large') && !is_paged() && !in_array(tails_get_theme_option('body_style'), array('fullwide', 'fullscreen'))) {
			the_post();
			get_template_part( 'content', 'excerpt' );
		}
		
		?><div class="<?php echo esc_attr($tails_classes); ?>"><?php
	}
	while ( have_posts() ) { the_post(); 
		if ($tails_sticky_out && !is_sticky()) {
			$tails_sticky_out = false;
			?></div><div class="<?php echo esc_attr($tails_classes); ?>"><?php
		}
		get_template_part( 'content', $tails_sticky_out && is_sticky() ? 'sticky' : 'classic' );
	}
