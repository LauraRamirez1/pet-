...
( function( $ ) {
	...
	var $style = $('#tails-color-scheme-css'),
		api = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length == 0 ) {
		$style = $('head').append( '<'+'style type="text/css" id="tails-color-scheme-css" />' ).find('#tails-color-scheme-css');
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-other-controls', function( obj ) {
			...
			if (id == 'body_style') {
				...
				$('.sc_layouts_logo .logo_text').html( tails_prepare_macros(val) );

			} else if (id == 'blogdescription') {
				$( '.sc_layouts_logo .logo_slogan' ).html( tails_prepare_macros(val) );

			} else if (id == 'copyright') {
				$( '.copyright_text' ).html( tails_prepare_macros(val) );
