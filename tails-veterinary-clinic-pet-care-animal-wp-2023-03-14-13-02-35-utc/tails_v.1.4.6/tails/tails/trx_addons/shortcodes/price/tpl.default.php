...
?>><?php
	...
	if (!empty($args['image'])) {
		...
		?><img src="<?php echo esc_url($args['image']); ?>" alt="'.esc_attr__('image', 'tails').'"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
		if (!empty($args['link'])) {
			?></a><?php
		} else {
			?></div><?php
	...
	if (!empty($args['link']) && !empty($args['link_text'])) {
		?><a href="<?php echo esc_url($args['link']); ?>" class="sc_price_link" target="_blank"><?php echo esc_html($args['link_text']); ?></a><?php
	}
