...
?>><?php
	...
	if ((int)$args['columns'] > 1) {
		?><div class="sc_icons_columns sc_item_columns <?php echo esc_attr(trx_addons_get_columns_wrap_class()); ?> columns_padding_bottom"><?php
	}
	...
	foreach ($args['icons'] as $item) {
		...
		?><div class="sc_icons_item<?php echo !empty($item['link']) ? ' sc_icons_item_linked' : ''; ?>"><?php
			if (!empty($item['image'])) {
				...
				?><div class="sc_icons_image"><img src="<?php echo esc_url($item['image']); ?>" alt="'.esc_attr__('image', 'tails').'"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
			} else {
				$icon = !empty($item['icon_type']) && !empty($item['icon_' . $item['icon_type']]) && $item['icon_' . $item['icon_type']] != 'empty' ? $item['icon_' . $item['icon_type']] : '';
				if (!empty($icon)) {
					$svg = '';
					if ((int)$args['icons_animation'] > 0 && ($svg = trx_addons_get_file_dir('css/svg-icons/'.trx_addons_esc($icon).'.svg')) != '') {
						$item['icon_type'] = 'svg';
						$svg_present = true;
					} else if (strpos($icon_present, $item['icon_type'])===false)
						$icon_present .= (!empty($icon_present) ? ',' : '') . $item['icon_type'];
					?><div id="<?php echo esc_attr($args['id'].'_'.trim($icon)); ?>" class="sc_icons_icon sc_icon_type_<?php echo esc_attr($item['icon_type']); ?><?php echo empty($svg) ? ' '.esc_attr($icon) : ''; ?>"
			...
			if (!empty($item['description'])) {
				...
				?><div class="sc_icons_item_description"><?php
					foreach ($item['description'] as $str) {
						?><span><?php echo tails_show_layout($str); ?></span><?php
					}
				?></div><?php
			}
			if (!empty($item['link'])) {
				?><a href="<?php echo esc_url($item['link']); ?>" class="sc_icons_item_link"></a><?php
			}
		?></div><?php
		if ((int)$args['columns'] > 1) {
			?></div><?php
